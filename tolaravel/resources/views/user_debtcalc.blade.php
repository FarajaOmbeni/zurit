...
<div class="container mt-5">
  ...
  <!-- Debt Form -->
  <div id="debt-form" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); -ms-transform: translate(-50%, -50%); background-color: #fff; padding: 20px; z-index: 5; width: 80%; max-width: 500px;">
  <form action="{{ route('debt_store') }}" method="POST">
    ...
    <div class="form-group">
        ...
        <input type="text" id="other-category" name="debt[other-category][name]" style="display: none;"><br>
            ...
            <form action="extraPayment_store" method="post">
                ...
                <div class="mb-3">
                    ...
                    <div class="range-container">
                        <input type="range" id="range" min="0" max="100000" value="0">
                    </div>
  ...
  <div class="card-body">
    <div class="d-flex justify-content-between align-items-center">
      <div role="progressbar" aria-valuenow="{{ number_format($principalPaid / $totalDebt * 100, 2) }}" aria-valuemin="0" aria-valuemax="100" style="--value:{{ number_format($principalPaid / $totalDebt * 100, 2) }}" data-percentage="{{ number_format($principalPaid / $totalDebt * 100, 2) }}"></div>
...
<div class="card mt-3">
  <div class="card-body">
    ...
    <select id="debtDropdown" class="form-select form-select-sm float-end">
      @foreach($debts as $debt)
        <option value="{{ $debt->current_balance * ($debt->interest_rate / 100 / 12) }}">{{ $debt->debt_name }}</option>
      @endforeach
...
<div class="card mt-3">
  <div class="card-body">
    ...
    <select id="paymentDropdown" class="form-select form-select-sm float-end" onchange="updatePayment()">
      @foreach($debts as $debt)
        <option value="{{ $debt->minimum_payment + ($debt->current_balance * ($debt->interest_rate / 100 / 12)) }}">{{ $debt->debt_name }}</option>
      @endforeach
