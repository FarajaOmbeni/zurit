            ...
            <div id="content" class="p-4 p-md-5 pt-5">
                ...
                <div class="book_form" id="editUserForm">
                    <form method="POST" action="{{ route('users.update', $user->id) }}">
                        ...
                        <input value="{{ $user->name }}" type="text" name="name" id="name" class="input"
                            required>

                        <label for="email" class="label">Email:</label>
                        <input value="{{ $user->email }}" type="email" name="email" id="email" class="input"
                            required>

                        <label for="phone" class="label">Phone:</label>
                        <input value="{{ $user->phone }}" type="text" name="phone" id="phone" class="input"
                            required>

                        <label for="role" class="label">Role:</label>
                        <input value="{{ $user->role }}" type="text" name="role" id="role" class="input"
                            required>
